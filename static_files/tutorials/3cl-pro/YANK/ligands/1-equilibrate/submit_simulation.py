...
def get_onq():
  ...
  for job in cuda_jobs:
    submit_script = '.'.join(job['Output_Path'].split(':')[-1].split('.')[:-1]) + '.job'
...
parser = argparse.ArgumentParser(\
  description = 'Submits MD simulations to Bridges or CCB.')
...
job_name = os.path.basename(args.system_name)
...
# Run MD
cd {script_path}
python ./run_equilibration.py --system_name {args.system_name} --simulation_time {args.simulation_time}
'''

# Write and submit scripts
submit_F = open(job_name+'_MD.job','w')
submit_F.write(submit_script)
submit_F.close()

if not args.dry:
  if cluster in ['Bridges','Comet']:
    os.system(f'sbatch {job_name}_MD.job')
  elif cluster=='CCB':
    os.system(f'qsub {job_name}_MD.job')
