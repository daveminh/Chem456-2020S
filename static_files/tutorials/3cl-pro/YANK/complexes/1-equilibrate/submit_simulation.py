...
#PBS -o {args.system_name}.out
#PBS -e {args.system_name}.err
...
submit_F = open(args.system_name+'_MD.job','w')
submit_F.write(submit_script)
submit_F.close()

if not args.dry:
  if cluster in ['Bridges','Comet']:
    os.system(f'sbatch {args.system_name}.job')
  elif cluster=='CCB':
    os.system(f'qsub {args.system_name}.job')
